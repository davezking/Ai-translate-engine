...
export const onRequestPost: PagesFunction<Env, string, AuthedData> = async (context) => {
  const body = (await context.request.json().catch(() => null)) as { sourceEnglish?: unknown } | null;
  const sourceEnglish = typeof body?.sourceEnglish === "string" ? body.sourceEnglish.trim() : "";
  if (!sourceEnglish) {
    return Response.json({ error: "sourceEnglish is required and cannot be empty" }, { status: 400 });
  }
