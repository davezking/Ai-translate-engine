...
9. **Gemini calls are resilient by design.** `functions/lib/gemini.ts` retries transient failures (429/5xx, including the "high demand" 503) with backoff, up to 4 attempts on `gemini-3.6-flash`. Don't remove this without checking why a call is failing first. There is deliberately **no fallback to a second model** — an earlier attempt at one used `gemini-3.6-flash-lite`, which doesn't exist for this API (confirmed via a live 404). Don't add a fallback model without first confirming its exact ID against a real `ListModels` call for this project's key/API version.
