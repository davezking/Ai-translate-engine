...
9. **Gemini calls are resilient by design.** `functions/lib/gemini.ts` walks a model chain — `gemini-3.6-flash` → `gemini-3.5-flash-lite` → `gemini-3.1-flash-lite` — on transient failures (429/5xx, including the "high demand" 503) or a 404 (model ID not valid for this API). Every model but the last gets one attempt before advancing; the last gets the full retry-with-backoff budget (4 attempts). All three IDs are confirmed against a live `ListModels` call for this project's key/API version (2026-09-04). A 404 still advances the chain rather than failing outright: `gemini-3.6-flash-lite`, an earlier fallback guess that was never checked against `ListModels`, turned out not to exist for this API (confirmed via a live 404), so this guards against the next model rename or deprecation, not because the current three are in doubt. If you add or change a model in the chain, confirm its exact ID against `ListModels` first rather than guessing.
