...
- **Long articles used to dilute both QA and retrieval — mitigated in Phase 5.1.** QA and retrieval used to run once per article, over the whole reassembled text: on a ~3000-word article the single retrieval query was a blurry average of several sub-topics, the 4 retrieved lessons were a small fraction of the prompt, and Gemini's attention is known to weaken toward the end of a long output. `runQaPipeline` (`functions/lib/qaPipeline.ts`) now retrieves and QAs each chunk independently instead, at the cost of ~N Gemini calls instead of 1 per article; one chunk's QA pass failing never fails the others (its ord is reported in `failedOrds` and it keeps its plain translation). Retrieval also no longer always returns top-N regardless of fit: `retrieveLessons` (`functions/lib/retrieval.ts`) drops matches below a tunable relevance floor (`QA_RETRIEVAL_MIN_SCORE`, default permissive — see the function's own doc comment for why a confident-looking default was deliberately avoided without real seed-corpus score data) before they're resolved against D1. Open: the floor's default should be tuned upward once the 50+ seed examples are loaded and real match scores are visible in the per-chunk retrieval log.
